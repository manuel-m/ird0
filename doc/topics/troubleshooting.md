...
- `No authorized key found for user`

**Diagnostics:**

1. Check SFTP server logs:
```bash
docker compose logs sftp-server | grep Authentication
```

2. Verify authorized_keys file:
```bash
cat keys/authorized_keys
# Should have: ssh-rsa <key-data> username
```
...
**authorized_keys file format wrong:**
- Must have three fields: `<key-type> <key-data> <username>`
- Username must NOT include @host
- Fix format:
  ```bash
  awk '{print $1" "$2" policyholder-importer"}' keys/sftp_client_key.pub > keys/authorized_keys
...
- Verify key pair matches:
  ```bash
  ssh-keygen -y -f keys/sftp_client_key > /tmp/public_from_private.pub
  diff keys/sftp_client_key.pub /tmp/public_from_private.pub
  ```

**Ed25519 key used (not supported):**
- Generate RSA key instead:
  ```bash
  ssh-keygen -t rsa -b 2048 -f keys/sftp_client_key -N ""
  ```

**SFTP server needs restart:**
- Keys loaded at startup only
- Restart after authorized_keys changes:
  ```bash
...
**Cause:** SFTP server host key changed (regenerated)
